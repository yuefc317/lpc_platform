...
int serial_init (void)
{
	...
	int unum;
	UART_REGS_T *puregs = (UART_REGS_T *) CONFIG_SYS_UART_SEL;
...
int serial_getc (void)
{
	...
	while ((CONFIG_SYS_UART_SEL->lsr & UART_LSR_RDR) == 0);

	return (int) (CONFIG_SYS_UART_SEL->dll_fifo & 0xFF);
}
...
void serial_putc (const char c)
{
	...
	while ((CONFIG_SYS_UART_SEL->lsr & UART_LSR_THRE) == 0);

	CONFIG_SYS_UART_SEL->dll_fifo = (u32) c;
...
{
	/* Wait for a character from the UART */
	if ((CONFIG_SYS_UART_SEL->lsr & UART_LSR_RDR) == 0)
	{
		return 0;
	}
