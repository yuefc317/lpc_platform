...
#if 1 
	ENETMAC->sa [2] = (unsigned long) (phyhwdesc.mac[0] | (phyhwdesc.mac[1] << 8));
	ENETMAC->sa [1] = (unsigned long) (phyhwdesc.mac[2] | (phyhwdesc.mac[3] << 8));
	ENETMAC->sa [0] = (unsigned long) (phyhwdesc.mac[4] | (phyhwdesc.mac[5] << 8));
...
for (i=0; i<6; i++)
{
	...
	ENETMAC->sa [2] = (unsigned long) (mac[0] | (mac[1] << 8));
	ENETMAC->sa [1] = (unsigned long) (mac[2] | (mac[3] << 8));
	ENETMAC->sa [0] = (unsigned long) (mac[4] | (mac[5] << 8));
#endif
...
*/
	printf ("\tHW MAC address:  "
		...
		phyhwdesc.mac[2], phyhwdesc.mac[3],
		phyhwdesc.mac[4], phyhwdesc.mac[5] );
