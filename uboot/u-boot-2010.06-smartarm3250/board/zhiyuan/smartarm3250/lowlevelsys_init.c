...
#define zhiyuan_XMKSTR(x)  #x
#define zhiyuan_MKSTR(x)  zhiyuan_XMKSTR(x)
//#define zhiyuan_MKX(x) zhiyuan
...
void phy3250_get_board_info(void)
{
	...
	if (phyhwdesc.fieldvval != PHY_HW_VER_VAL)
	{
		...
		mac_add = zhiyuan_MKSTR(CONFIG_ETHADDR); //transform MAC address macro to string
		printf("amc address is : %s\n ", mac_add);
		for(i = 0; i < 6; i++){ 
			if( mac_add[3*i]>='a' && mac_add[3*i]<='f' )
				phyhwdesc.mac[i] = (mac_add[3*i] - 'a' + 10) << 4;
			else if ( mac_add[3*i]>='A' && mac_add[3*i]<='F' )
				phyhwdesc.mac[i] = (mac_add[3*i] - 'A' + 10) << 4;
			else
				phyhwdesc.mac[i] = (mac_add[3*i] - '0') << 4;

			if( mac_add[3*i+1]>='a' && mac_add[3*i+1]<='f' )
				phyhwdesc.mac[i] |= (mac_add[3*i+1] - 'a' + 10);
			else if ( mac_add[3*i+1]>='A' && mac_add[3*i+1]<='F' )
				phyhwdesc.mac[i] |= (mac_add[3*i+1] - 'A' + 10);
			else
				phyhwdesc.mac[i] |= (mac_add[3*i+1] - '0');
		}
#else
		printf("*******WARNING :use default MAC address\n");
		for(i = 0; i < 6; i++)
				phyhwdesc.mac[i] = i << 2;
#endif
